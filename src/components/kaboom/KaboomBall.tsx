...
const KaboomBallComponent: React.FC<KaboomBallProps> = ({
  ...
  if (tile.revealed) {
    ...
    if (tile.type === 'bomb') {
      if (tile.isDefused) {
        return (
          <div
            ...
            className={`relative w-full aspect-square ${outerRadius} flex items-center justify-center border-2 border-emerald-400 bg-[#03150d]/90 shadow-[0_0_20px_rgba(16,185,129,0.8),inset_0_0_10px_rgba(16,185,129,0.45)] animate-pulse overflow-visible z-20 ${rippleClass}`}
            style={rippleStyle}
          >
      ...
      return (
        <div
          id={`kaboom-tile-${tile.id}`}
          className={`relative w-full aspect-square ${outerRadius} flex items-center justify-center transition-all duration-300 overflow-visible z-20 ${
            tile.isDetonated
              ? 'border-2 border-red-500 bg-red-950/95 shadow-[0_0_30px_rgba(239,68,68,0.95),inset_0_0_14px_rgba(239,68,68,0.6)] animate-bounce'
        ...
        >
          <div className={`absolute ${insetClass} ${innerRadius} border border-red-500/60 flex items-center justify-center bg-black/40 overflow-visible`}>
            ...
            <div
              className={`absolute -inset-3 sm:-inset-4 z-30 flex items-center justify-center select-none pointer-events-none overflow-visible ${
  ...
  if (isGameOver) {
    return (
      <div
        id={`kaboom-tile-${tile.id}`}
        className={`relative w-full aspect-square ${outerRadius} flex items-center justify-center opacity-55 border border-zinc-700/50 bg-zinc-900/40 overflow-visible ${rippleClass}`}
        style={rippleStyle}
      >
        <div
